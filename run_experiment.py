...
def run_experiment(experiment_detals, experiment, timing_key, verbose, timings):
    ...
    for details in experiment_detals:
        logger.info("Running {} experiment: {}".format(
            timing_key, details.env_readable_name))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--policy', action='store_true',
                        help='Run the policy iteration experiment')
    parser.add_argument('--value', action='store_true',
                        help='Run the value iteration experiment')
    parser.add_argument('--q', action='store_true',
                        help='Run the Q-Learner experiment')
    parser.add_argument('--all', action='store_true',
                        help='Run all experiments')
    parser.add_argument('--plot', action='store_true',
                        help='Plot data results')
    ...
    seed = args.seed
    if seed is None:
        # The openAI gym code does not like unit64
    ...
    envs = [
        ...
        {
            'env': environments.get_millionaire_environment(),
            'name': 'millionaire',
            'readable_name': 'Millionaire',
        }
        # },
        # {
    ...
    if args.policy or args.all:
        run_experiment(
            experiment_details, experiments.PolicyIterationExperiment, 'PI', verbose, timings)

    if args.value or args.all:
        run_experiment(
            experiment_details, experiments.ValueIterationExperiment, 'VI', verbose, timings)

    if args.q or args.all:
        run_experiment(experiment_details,
                       experiments.QLearnerExperiment, 'Q', verbose, timings)
